...
function abrirModalAtualizacao(event) {
    ...
    // garante que estamos pegando o botão, não algum filho
    const botao = event.currentTarget || event.target;
    const artigoId = botao.dataset.id;

    if (!artigoId) {
        console.error('Nenhum data-id encontrado no botão de atualização');
        return;
    }

    // 1. Encontra o card do artigo
    const cardElement = document.querySelector(`.artigo-card[data-id='${artigoId}']`);
    if (!cardElement) {
        console.error('Card do artigo não encontrado para id:', artigoId);
        return;
    }
    ...
    // 3. Categoria
    const categoriaParagrafo = Array.from(
        cardElement.querySelectorAll('.card-content p')
    ).find(p => p.textContent.includes('Categoria:'));

    let categoriaNome = '';
    if (categoriaParagrafo) {
        categoriaNome = categoriaParagrafo.textContent
            .replace('Categoria:', '')
            .trim();
    }

    const selectCategoria = form.querySelector('#edit-id_categoria');
    const optionToSelect = Array.from(selectCategoria.options)
        .find(opt => opt.text.trim() === categoriaNome);
    ...
    // 4. DOI   pega o link "Ver DOI"
    // usa o primeiro .btn-secondary que for <a href="...">
    const doiAnchor = cardElement.querySelector('.btn-secondary[href]');
    const doiLink = doiAnchor ? doiAnchor.href : '';
    form.querySelector('#edit-link_doi').value = doiLink;

    // 5. Imagem atual
    const imgTag = cardElement.querySelector('img');
    const imgUrl = imgTag ? imgTag.src : '';
    form.querySelector('#edit-url_imagem_existente').value = imgUrl;

    const imgPreview = form.querySelector('#edit-img-preview');
    if (imgUrl) {
    ...
    // 6. PDF   botão "Baixar PDF"   .btn-primary
    const pdfAnchor = cardElement.querySelector('.btn-primary[href]');
    const pdfLink = pdfAnchor ? pdfAnchor.href : '';
    form.querySelector('#edit-link_pdf_input').value = pdfLink;

    // 7. Exibir   Ocultar
    const isExibir = !cardElement.classList.contains('oculto');
    form.querySelector('#edit-exibir').checked = isExibir;

    // 8. Limpa campos de nova imagem   PDF
    form.querySelector('#edit-url_imagem_input').value = '';
    form.querySelector('#edit-imagem_file_input').value = '';
    form.querySelector('#edit-pdf_file_input').value = '';

    // 9. Abre o modal
    modalAtualizacao.style.display = 'flex';
}
